...
func Run() (*config.Config, error) {
	...
	err = generateUIPackage(&cfg)
	if err != nil {
		return nil, fmt.Errorf("failed to generate ui package: %w", err)
	}

	err = generateCRUDPackage(cfg.App.Name)
	if err != nil {
		return nil, fmt.Errorf("failed to generate crud package: %w", err)
	}
	...
	fmt.Println("Generated: .uca/registry/registry.go")
	...
	err = generateDevMain(&cfg)
	if err != nil {
		return nil, fmt.Errorf("failed to generate dev main: %w", err)
	}

	err = generateAutoTests(&cfg)
	if err != nil {
		return nil, fmt.Errorf("failed to generate auto tests: %w", err)
	}

	err = generateGitIgnore()
	if err != nil {
		fmt.Printf("[UCA] Warning: failed to generate .gitignore: %v\n", err)
	}

	// 10. Sync custom dependencies if deps.json exists
	if _, err := os.Stat("deps.json"); err == nil {
		err = deps.SyncFromFile("deps.json")
		if err != nil {
			fmt.Printf("[UCA] Warning: failed to sync dependencies from deps.json: %v\n", err)
		}
	}
...
func generateGitIgnore() error {
	...
	content := `# Uca framework hidden sandbox
.uca/

# Environment variables (secrets)
.env

# Generated manifest templates
.env.uca
...
func GenerateEnvUca(cfg *config.Config) error {
	var content strings.Builder
	content.WriteString("# --- Auto-generated by Uca (Template for required environment keys) ---\n\n")
	
	for _, key := range cfg.App.Keys {
		content.WriteString(fmt.Sprintf("%s=\n", key))
	}
	
	content.WriteString("\n# Internal Framework Ports\n")
	content.WriteString(fmt.Sprintf("UCA_FRONTEND_PORT=%d\n", cfg.App.Port.Frontend))
	content.WriteString(fmt.Sprintf("UCA_BACKEND_PORT=%d\n", cfg.App.Port.Backend))
	content.WriteString(fmt.Sprintf("UCA_AI_PORT=%d\n", cfg.App.Port.AI))

	return os.WriteFile(".env.uca", []byte(content.String()), 0644)
}

func GenerateDepsJson() error {
	if _, err := os.Stat("deps.json"); err == nil {
		return nil // Don't overwrite if exists
	}
...
func reconcileFiles(cfg *config.Config) error {
	...
	dirs := []string{"pages", "services", "agents", "assets", "modules"}
	for _, d := range dirs {
