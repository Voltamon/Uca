...
func Run() (*config.Config, error) {
	...
	err = generateUIPackage()
	if err != nil {
		return nil, fmt.Errorf("failed to generate ui package: %w", err)
	}

	err = generateContextPackage()
	if err != nil {
		return nil, fmt.Errorf("failed to generate context package: %w", err)
	}

	err = generateAgentPythonPackage()
	if err != nil {
		return nil, fmt.Errorf("failed to generate agent python package: %w", err)
	}
